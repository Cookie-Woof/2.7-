...
def save_screenshot(path):
    """
    Saves the screenshot you took earlier to a file.
    If you haven't taken a screenshot yet, it'll tell you to do that first.
...
def delete_command(path):
    ...
    Just give it the full path to the file and it's gone.
...
def copy_file_command(source_path, dest_path):
    ...
    If dest_path doesn't exist, it creates it. If it does exist, it overwrites it.
    """
    try:
...
def excute_command(path):
    """
    Runs an executable file (like .exe programs).
    It starts the program in the background and returns right away,
    so the server doesn't freeze waiting for the program to finish.
    """
    try:
...
def receive_message(client_socket):
    """Receives message with 4-byte length prefix and verifies byte count"""
...
def main():
    """
    This is where everything starts. It sets up the server and handles clients.
    ...
    4. When a client connects, we receive their commands and send back responses
